...
def list_documents(
    ...
    filter_field: Annotated[str | None, typer.Option("--filter-field", help="Meta field to filter on.")] = None,
    filter_value: Annotated[str | None, typer.Option("--filter-value", help="Value to match.")] = None,
    as_json: Annotated[bool, typer.Option("--json", help="Output as JSON.")] = False,
) -> None:
    """List documents in the configured document store."""
    try:
        data = DocumentLister().list(limit=limit, filter_field=filter_field, filter_value=filter_value)
